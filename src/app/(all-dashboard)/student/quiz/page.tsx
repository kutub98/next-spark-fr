...
//       question: question._id, // <-- use `question` as per IAnswer
...
//       ...prev,
//       [question._id]: newAnswer, // key can still be question._id
//     }));
//   };
...
"use client";
import QuizzesBasedOnEvent from "@/app/components/quiz/quiz";
...
const Quizzes = () => {
  ...
  return (
    <div className="">
      {allEvents.length === 0 ? (
        <div>
          <h1>No events Found for Quiz</h1>
        </div>
      ) : (
        allEvents.map((item, key) => (
          <div key={key} className="">
            <QuizzesBasedOnEvent eventId={item._id} />
          </div>
        ))
      )}
    </div>
