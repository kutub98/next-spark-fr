...
export default function AdminDashboard() {
  ...
  }, [users, filter, customDate]); // rerun when filter or date changes
  ...
  // 🔍 Improved filter logic (includes current month)
  const filterDataByRange = (data: any[], dateField: string) => {
    const now = new Date();
    let startDate: Date;
    ...
    if (customDate) {
      ...
      switch (filter) {
        ...
        case "week":
          ...
          break;
        case "month":
          startDate = subDays(startOfDay(now), 29); // rolling 30 days
          break;
        case "currentMonth":
          startDate = startOfMonth(now); // calendar current month
    ...
    return data.filter((item) => {
      const created = parseISO(item[dateField]);
      return isAfter(created, startDate) && isBefore(created, endDate);
    });
  };
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <defs className="bg-yellow-500">
                    ...
                    <linearGradient
                      id="colorParticipations"
                      x1="0"
                      y1="0"
                      x2="0"
                      y2="1"
                    >
                      <stop offset="0%" stopColor="#ffc658" stopOpacity={0.3} />
                  ...
                  {/* Lines with animation + area fill */}
                  <Line
                    type="monotone"
                    dataKey="registrations"
                    stroke="#8884d8"
